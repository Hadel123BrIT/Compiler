...
//    private final PipeSymbolTable pipeSymbolTable = new PipeSymbolTable();
    ...
    public Program visitProgram(AngularParser.ProgramContext ctx) {
        ...
        componentSymbolTable.getComponents().forEach((name, symbol) -> System.out.println(symbol.getSymbolType() + ": " + name + " (Selector: " + symbol.getSelector() + ")"));
        return program;
    }
...
//--------------------------------------------------
    ...
    public ComponentProperty visitComponentProperty(AngularParser.ComponentPropertyContext ctx)
    {
        ...
        updateLineNumber(ctx);
//        System.out.println(componentSymbolTable.getComponents().get("AppComponent").toString());
//        System.exit(33);
        if (ctx.SELECTOR() != null && ctx.StringLiteral() != null) {
            ...
            if (currentComponentName != null) {
                ...
                System.out.println(currentComponentName);
                System.exit(-1);
            ...
            componentProperty.setTemplate(visitTemplate(ctx.template()));
    ...
    public VariableStatement visitVariableStatement(AngularParser.VariableStatementContext ctx) {
        ...
        if (ctx.Identifier() != null) {
            for(int V = 0 ; V < ctx.Identifier().size() ; V++ ){
                ...
                templateVariableScopeTable.getScopes().peek().put(varName, variableSymbol);
                variableStatement.setIdentifier(varName);
            }
        }
...
// Check symbol table
            System.out.println(functionsSymbolTable.checkIfExist(funName, currentLine));
...
//-------------------------------------------------------------
    ...
    public FunctionDeclaration visitFunctionDeclaration(
            AngularParser.FunctionDeclarationContext ctx) {
        ...
        if (ctx.block() != null) {
            functionDeclaration.setBlock((Block) visitBlock(ctx.block()));
        }
...
//------------------------------------------------------------
    ...
    public ArrayElement visitArrayElement(AngularParser.ArrayElementContext ctx) {
        ...
        if (ctx.object() != null) {
            arrayElement.setObject((AST.Object) visitObject(ctx.object()));
        }
        if (ctx.value() != null) {
            arrayElement.setValue((Value) visitValue(ctx.value()));
        }
